...
Train_Data$Default=ifelse(Train_Data$Default=="Good",0,1)
Train_Data$Default=as.factor(Train_Data$Default)
Test_Data$Default=ifelse(Test_Data$Default=="Good",0,1)
Test_Data$Default=as.factor(Test_Data$Default)


trainCt=trainControl(method = "repeatedcv",number=10,repeats = 3)
SVM.Model=train(Default~.,data=Train_Data,method="svmLinear",
                trControl=trainCt,
                preProcess=c("center","scale"),
                tuneLength=10)

testpredict=predict(SVM.Model,newdata=Test_Data);testpredict

cmat=confusionMatrix(testpredict,Test_Data$Default);cmat
